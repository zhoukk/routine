...
local sharedata = require "sharedata_corelib"
local table = table
...
local function collectobj()
	while true do
		pixel.sleep(600 * 100)	-- sleep 10 min
		collectgarbage()
...
function init()
	pixel.fork(collectobj)
	pixel.name "sharedatad"
end

function request:new(name, t)
	local dt = type(t)
	local value
	if dt == "table" then
		value = t
...
function post:confirm(cobj)
	if objmap[cobj] then
		sharedata.host.decref(cobj)
	end
end

function request:update(name, t)
	local v = pool[name]
	local watch, oldcobj
	if v then
		watch = v.watch
...
function request:monitor(name, obj)
	local v = assert(pool[name])
	if obj ~= v.obj then
		return v.obj
	end
	...
	pool_count[name].n = n
	table.insert(v.watch, pixel.response())
end
