...
struct socket_req {
	int req;
	union {
		struct _listen_req listen;
		struct _open_req open;
		struct _bind_req bind;
		struct _start_req start;
		struct _close_req close;
		struct _send_req send;
		struct _opt_req opt;
		struct _udp_req udp;
		struct _setudp_req setudp;
		struct _sendudp_req sendudp;
	} u;
};
...
static void socket_nonblocking(int fd) {
...
static void socket_send_req(struct socket_req *req) {
	for (;;) {
		int n = write(S.sendctl_fd, (void *)req, sizeof(*req));
		if (n < 0) {
			if (errno != EINTR) {
				fprintf(stderr, "socket_send_req error:%d.\n", errno);
				break;
			}
			continue;
		}
...
static int socket_recv_req(struct socket_req *req) {
	for (;;) {
		int n = read(S.recvctl_fd, (void *)req, sizeof(*req));
		if (n < 0) {
			if (errno != EINTR) {
				fprintf(stderr, "socket_recv_req error:%d.\n", errno);
				return -1;
			}
			continue;
		}
...
static int _socket_bind(const char *host, int port, int protocol, int *family) {
	int fd;
	int status;
	int reuse = 1;
...
static int _socket_listen(const char *host, int port) {
	int family = 0;
...
static void socket_free_buffer_list(struct buffer_list *list) {
...
static void socket_force_close(struct socket *sock, struct socket_message *ret) {
	ret->id = sock->id;
	ret->ud = sock->ud;
	ret->data = 0;
	ret->size = 0;
...
static inline int socket_send_object_init(struct socket_send_object *so, void *data, int size) {
	if (size < 0) {
...
static int socket_send_buffer_list(struct socket *sock, struct buffer_list *list, struct socket_message *ret) {
	while (list->head) {
...
static int socket_buffer_list_complete(struct buffer_list *list) {
	struct buffer *wb = list->head;
	if (wb == 0) {
		return 1;
	}
...
static int socket_forward_tcp(struct socket *sock, struct socket_message *ret) {
	int n;
	int sz = sock->p.size;
...
static int socket_forward_udp(struct socket *sock, struct socket_message *ret) {
	uint8_t *data;
...
static struct buffer *socket_append_buffer_list(struct buffer_list *list, struct _send_req *req, int size, int n) {
...
static void socket_append_sendbuffer(struct socket *sock, struct _send_req *req, int n) {
	struct buffer *buf;
...
static void socket_append_udp_sendbuffer(struct socket *sock, struct _send_req *req, const uint8_t udp_address[UDP_ADDRESS_SIZE]) {
	struct buffer *buf;
	if (req->priority == SOCKET_PRIORITY_HIGH) {
...
static socklen_t udp_socket_address(struct socket *sock, const uint8_t udp_address[UDP_ADDRESS_SIZE], union sockaddr_all *sa) {
	uint16_t port = 0;
...
static int socket_handle_req(struct socket_message *msg) {
	...
	default:
		fprintf(stderr, "socket_handle_req error:%d.\n", errno);
	}
	return -1;
}
...
int socket_init(socket_dispatch dispatch, socket_alloc alloc) {
	int i;
	int fd[2];
	memset(&S, 0, sizeof(S));
...
static void freebuffer(void *data, int size) {
	struct socket_send_object so;
...
static void clear_closed(int id, int type) {
...
static int _socket_has_ctrl(void) {
	struct timeval tv = { 0, 0 };
	int ret;
	FD_SET(S.recvctl_fd, &S.rfds);
	ret = select(S.recvctl_fd + 1, &S.rfds, 0, 0, &tv);
	if (ret == 1) {
		return 1;
	}
	return 0;
...
const struct socket_udp_address *socket_udpaddress(struct socket_message *m, int *address_size) {
	...
	return (const struct socket_udp_address *)udp_address;
}
