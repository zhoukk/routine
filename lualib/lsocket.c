...
static int lpushbuffer(lua_State *L) {
	...
	if (!node) {
		int i;
		int size = 2;
		bp->pool = (struct buffer_node *)pixel_alloc(0, size*sizeof(struct buffer_node));
		for (i = 0; i < size - 1; i++) {
...
static int llisten(lua_State *L) {
	...
	uint32_t handle = pixel_handle(ctx);
...
static int ludp_send(lua_State *L) {
	...
	nsend = socket_udpsend(id, (struct socket_udp_address *)address, data, size);
	lua_pushinteger(L, (int)nsend);
	return 1;
}
...
static int lunpack(lua_State *L) {
	...
	if (sm->type == SOCKET_UDP) {
		int address_size = 0;
		const char *address = (const char *)socket_udpaddress(sm, &address_size);
		if (address) {
...
int luaopen_socket_c(lua_State *L) {
	...
	ctx = (struct pixel *)lua_touserdata(L, -1);
	if (!ctx) {
