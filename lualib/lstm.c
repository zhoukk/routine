...
// msg should alloc by pixel_alloc 
static struct stm_copy *
stm_newcopy(void * msg, int32_t sz) {
	struct stm_copy * copy = pixel_alloc(0, sizeof(*copy));
	copy->reference = 1;
	copy->sz = sz;
	copy->msg = msg;
...
stm_new(void * msg, int32_t sz) {
	struct stm_object * obj = pixel_alloc(0, sizeof(*obj));
