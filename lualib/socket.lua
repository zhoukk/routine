...
socket_handle[3] = function(id, size, data)
	local s = socket_pool[id]
	if s == nil then
		pixel.err("socket: drop package from %d\n", id)
		pixel.drop(data)
		return
	end
	-- pixel.dump(data, size)
	...
	if rrt == "number" then
		...
		if s.buffer_limit and size > s.buffer_limit then
			pixel.err("socket: buffer overflow fd=%d size=%d\n", id, size)
			s.buffer:free()
			c.close(id)
			return
		end
...
socket_handle[6] = function(id, size, data, address)
	...
	if s == nil or s.callback == nil then
		pixel.err("socket: drop udp package from %d\n", id)
		pixel.drop(data, size)
		return
	end
	local str = pixel.tostring(data, size)
	pixel.drop(data, size)
...
local function default_warning(id, size)
	...
	if last + 64 < size then	-- if size increase 64K
		s.warningsize = size
		pixel.err("WARNING: %d K bytes need to send out (fd = %d)", size, id)
	end
	s.warningsize = size
end
