...
static int ldesencode(lua_State *L) {
	...
	uint8_t *buffer = tmp;
	const uint8_t * text;
	size_t textsz, chunksz;
	uint8_t tail[8];
	des_key(L, SK);
	text = (const uint8_t *)luaL_checklstring(L, 2, &textsz);
	chunksz = (textsz + 8) & ~7;
...
static void read64(lua_State *L, uint32_t xx[2], uint32_t yy[2]) {
	size_t sz = 0;
	const uint8_t *x, *y;
	x = (const uint8_t *)luaL_checklstring(L, 1, &sz);
	if (sz != 8) {
		luaL_error(L, "Invalid uint64 x");
	}
	y = (const uint8_t *)luaL_checklstring(L, 2, &sz);
	if (sz != 8) {
...
static int pushqword(lua_State *L, uint32_t result[2]) {
	uint8_t tmp[8];
	tmp[0] = result[0] & 0xff;
...
/*
   ...
   string text
   */
static int lhmac_hash(lua_State *L) {
	uint32_t key[2];
	size_t sz = 0;
	uint8_t h[8];
	uint32_t htext[2];
