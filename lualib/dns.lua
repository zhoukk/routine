...
local function resolve(content)
	if #content < DNS_HEADER_LEN then
		-- drop
		pixel.err("Recv an invalid package when dns query\n")
		return
	end
	...
	if not resp then
		pixel.err("Recv an invalid tid when dns query\n")
		return
	end

	local question,left = unpack_question(content, left)
	if question.name ~= resp.name then
		pixel.err("Recv an invalid name when dns query\n")
		return
	end
...
function dns.server(server, port)
	...
	socket.udp_open(dns_server, server, port or 53)
	return server
end
