...
static struct uncomplete *save_uncomplete(lua_State *L, int fd) {
	struct queue *q = get_queue(L);
	int h = hash_fd(fd);
	struct uncomplete * uc = pixel_alloc(0, sizeof(struct uncomplete));
...
static void push_more(lua_State *L, int fd, uint8_t *buffer, int size) {
	...
	if (size < pack_size) {
		...
		uc->pack.buffer = pixel_alloc(0, pack_size);
...
static int filter_data_(lua_State *L, int fd, uint8_t * buffer, int size) {
	...
	if (uc) {
		...
		if (size == pack_size) {
			...
			lua_pushvalue(L, lua_upvalueindex(TYPE_DATA));
			lua_pushinteger(L, fd);
			void * result = pixel_alloc(0, pack_size);
...
static int lfilter(lua_State *L) {
	struct socket_message *message = lua_touserdata(L,2);
	...
	switch(message->type) {
		case SOCKET_DATA:
			...
			return filter_data(L, message->id, (uint8_t *)buffer, message->size);
		case SOCKET_OPEN:
...
static int lpack(lua_State *L) {
	...
	uint8_t * buffer = pixel_alloc(0, len + 2);
