...
static int pack(lua_State *L, void *data, size_t size) {
	struct mc_package * pack = pixel_alloc(0, sizeof(struct mc_package));
	pack->reference = 0;
	pack->size = (uint32_t)size;
	pack->data = data;
	struct mc_package ** ret = pixel_alloc(0, sizeof(*ret));
	*ret = pack;
...
/*
   ...
   */
static int mc_packlocal(lua_State *L) {
...
/*
   ...
   return lightuserdata, sizeof(struct mc_package *)
   */
static int mc_packremote(lua_State *L) {
	void * data = lua_touserdata(L, 1);
	size_t size = (size_t)luaL_checkinteger(L, 2);
	if (size != (uint32_t)size) {
		return luaL_error(L, "Size should be 32bit integer");
	}
	void * msg = pixel_alloc(0, size);
	memcpy(msg, data, size);
	return pack(L, msg, size);
}
static int mc_packstring(lua_State *L) {
	size_t size;
	const char * msg = luaL_checklstring(L, 1, &size);
	if (size != (uint32_t)size) {
		return luaL_error(L, "string is too long");
	}
	void * data = pixel_alloc(0, size);
...
/*
   ...
   */
static int mc_unpacklocal(lua_State *L) {
...
/*
   ...
   return mc_package *
   */
static int mc_bindrefer(lua_State *L) {
	struct mc_package ** pack = lua_touserdata(L,1);
...
/*
   lightuserdata struct mc_package *
   */
static int mc_closelocal(lua_State *L) {
	struct mc_package *pack = lua_touserdata(L,1);
	int ref = __sync_sub_and_fetch(&pack->reference, 1);
	if (ref <= 0) {
		pixel_alloc(pack->data, 0);
		pixel_alloc(pack, 0);
		if (ref < 0) {
...
/*
   ...
   */
static int mc_remote(lua_State *L) {
